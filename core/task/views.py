...
def step_approve(request, stepexid=None, reqid=None, sliceid=None):
    if request.method == 'GET':
        try:
            ...
            #print e
            return HttpResponseRedirect('/task/step_execution_table/')
    return HttpResponseRedirect('/task/step_execution_table/')
...
def home(request):
    tmpl = get_template('task/_index.html')
    c = Context({'active_app' : 'task', 'title'  : 'Monte Carlo Production Home'})
    return HttpResponse(tmpl.render(c))

def about(request):
    tmpl = get_template('task/_about.html')
    c = Context({'active_app' : 'task', 'title'  : 'Monte Carlo Production about', })
    return HttpResponse(tmpl.render(c))
...
def input_list_approve(request, rid=None):
    if request.method == 'GET':
        try:
                ...
                return   render(request, 'task/_reqdatatable.html', {
                   'active_app' : 'task',
...
class StepTemlateTable(datatables.DataTable):
    ...
    class Meta:
        ...
        bServerSide = True
        sAjaxSource = '/task/step_template_table/'

@datatables.datatable(StepTemlateTable, name='fct')
def step_template_table(request):
    qs = request.fct.get_queryset()
    request.fct.update_queryset(qs)
    return TemplateResponse(request, 'task/_datatable.html', {  'title': 'StepTemlates Table', 'active_app' : 'task', 'table': request.fct,
                'parent_template': 'task/_index.html'})
...
class StepExecutionTable(datatables.DataTable):
    ...
    class Meta:
        ...
        fnRowCallback = """
                        ...
                        {
                            $('td:eq(0)', nRow).html('<span style="float:right;"><a title="Approve this step" href="/task/step_approve/'+aData[0]+'/'+aData[2]+'/'+aData[1]+'">approve</a>'+
                                '&nbsp;</span>&nbsp;');
                        }"""
        sScrollX = '100em'
        sScrollY = '20em'
        ...
        bServerSide = True
        sAjaxSource = '/task/step_execution_table/'

@datatables.datatable(StepExecutionTable, name='fct')
def step_execution_table(request):
    qs = request.fct.get_queryset()
    request.fct.update_queryset(qs)
    return TemplateResponse(request, 'task/_datatable.html', {  'title': 'StepExecutions Table', 'active_app' : 'task', 'table': request.fct,
                                                                'parent_template': 'task/_index.html'})
