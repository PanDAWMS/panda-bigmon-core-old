...
@ensure_csrf_cookie
def jediUserJobset(request, produsername=None, jobsetid=None):
    """
        jediUserJobset -- view list of PanDA jobs in a dataTables table
    ...
    """
#    reverseUrl = 'api-datatables-jedi-user-jobset'
    reverseUrl = 'api-datatables-jedi-jobs-in-task'
    reverseUrlSmry = reverseUrl + '-smry'
    ### get URL prefix
    prefix = getPrefix(request)
...
#    filterFields = ['fJtaskID']
    ...
    data.update(getContextVariables(request))
    return render_to_response('pandajob/jedi/userjobset/jobsintask.html', data, RequestContext(request))


@ensure_csrf_cookie
def jediListUsers(request):
    """
        jediListUsers -- view list of PanDA jobs in a dataTables table
                            data from API jedi/userjobset
    ...
    """
    reverseUrl = 'api-datatables-jedi-list-users'
    ### get URL prefix
    prefix = getPrefix(request)
#    ### get reverse url of the data view
#    dataUrl = reverse(reverseUrl)
    res = QuerySetChain(
        Jobsactive4.objects.values('produsername').distinct(), \
        Jobsarchived4.objects.values('produsername').distinct(), \
        Jobsdefined4.objects.values('produsername').distinct(), \
        Jobswaiting4.objects.values('produsername').distinct(), \
    )
...
#            'columns': json_dumps(aoColumns), \
    ...
    data.update(getContextVariables(request))
    return render_to_response('pandajob/jedi/userlist/users.html', data, RequestContext(request))
